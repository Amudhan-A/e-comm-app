...
export function SearchContainer({ isCollapsed = false }: { isCollapsed?: boolean }) {
      ...
      sections: [
        {
          title: "Views",
          items: [
            { icon: <View size={16} className="text-neutral-50" />, label: "Inventory Overview", id: "products", isActive: true },
            {
        ...
        },
        {
          title: "Analytics",
          items: [
            { icon: <ChartBar size={16} className="text-neutral-50" />, label: "Stock Levels" },
            { icon: <Analytics size={16} className="text-neutral-50" />, label: "Top Selling" },
          ],
        },
      ],
    },
      ...
      sections: [
        {
          title: "Management",
          items: [
            { icon: <InProgress size={16} className="text-neutral-50" />, label: "Active Orders", id: "orders" },
          ...
          items: [
            {
              ...
              hasDropdown: true,
              children: [
                { icon: <Flag size={14} className="text-neutral-300" />, label: "Refund Requests" },
                { icon: <Flag size={14} className="text-neutral-300" />, label: "Large Orders" },
              ],
            },
          ],
        },
      ...
      sections: [
        {
          title: "Campaigns",
          items: [
            { icon: <AddLarge size={16} className="text-neutral-50" />, label: "New Campaign" },
            { icon: <FolderOpen size={16} className="text-neutral-50" />, label: "Active Promos" },
          ],
        },
      ],
    },
      ...
      sections: [
        {
          title: "System",
          items: [
            { icon: <SettingsIcon size={16} className="text-neutral-50" />, label: "General Settings" },
            { icon: <Security size={16} className="text-neutral-50" />, label: "Access Control" },
          ],
        },
      ],
    },
...
export function IconNavigation({
  ...
  const navItems = [
    ...
    { id: "tasks", icon: <Task size={16} />, label: "Orders" },
    { id: "projects", icon: <Folder size={16} />, label: "Marketing" },
    { id: "analytics", icon: <Analytics size={16} />, label: "Analytics" },
  ];

  return (
    <aside className="bg-black flex flex-col gap-2 items-center p-4 w-16 min-h-screen border-r border-neutral-800 rounded-l-2xl">
      {/* Logo */}
...
export function DetailSidebar({ activeSection, onTabChange }: DetailSidebarProps) {
  ...
  return (
    <aside
      className={`bg-black flex flex-col gap-4 items-start p-4 rounded-r-2xl transition-all duration-500 min-h-screen ${
        isCollapsed ? "w-16 min-w-16 !px-0 justify-center" : "w-64 md:w-80"
      }`}
      style={{ transitionTimingFunction: softSpringEasing }}
    >
      ...
      <div
        className={`flex-1 flex flex-col w-full overflow-y-auto transition-all duration-500 ${
      ...
      {!isCollapsed && (
        <div className="w-full mt-auto pt-2 border-t border-neutral-800">
          <div className="flex items-center gap-2 px-2 py-2 mb-4">
            <AvatarCircle />
            <div className="font-['Lexend:Regular',_sans-serif] text-[14px] text-neutral-50">Admin User</div>
            <button
              type="button"
...
export function SubMenuItem({ item, onItemClick }: { item: MenuItemT; onItemClick?: () => void }) {
  return (
              ...
              <div className="flex flex-col gap-1 mb-2">
                {item.children.map((child, childIndex) => (
                  <SubMenuItem
                    ...
                    item={child}
                    onItemClick={() => onItemClick?.(child.id)}
                  />
                ))}
              </div>
            )}
...
export function TwoLevelSidebar({ onTabChange }: TwoLevelSidebarProps) {
  const [activeSection, setActiveSection] = useState("dashboard");

  const handleSectionChange = (section: string) => {
      setActiveSection(section);
      if (section === 'dashboard') onTabChange?.('products');
      if (section === 'tasks') onTabChange?.('orders');
  }

  return (
    <div className="flex flex-row h-full">
      <IconNavigation activeSection={activeSection} onSectionChange={handleSectionChange} />
      <DetailSidebar activeSection={activeSection} onTabChange={onTabChange} />
    </div>
  );
}
...
export function SidebarFixedFrame() {
  return (
